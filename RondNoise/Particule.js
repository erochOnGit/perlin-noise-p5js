function Particule() {
  ...
  this.showPhyllo = function(posx, posy, color, opacity, n, angle, c, size) {
    let a = n * Math.PI * (3 - Math.sqrt(angle));
    let r = c * Math.sqrt(n);

    let x = r * (noise(n) * 50) * Math.cos(a) + posx;
    let y = r * (noise(n) * 50) * Math.sin(a) + posy;

    let rdmR = noise(n) * color;
    let saturation = noise(n) * 200 * n;
    let brightness = noise(n) * 255;
    let rdmA = noise(this.position.x) * opacity;
    let rdmS = noise(this.position.y) * 50;

    colorMode(HSB, 150);
    stroke(rdmR, 200 * (n / 2), brightness, rdmA);
    strokeWeight(size / (n / 10));
    point(x, y);
    // this.updatePrev();
  };
